...
import { Club, type IClub } from '../models/Club';
...
type ClubDoc = HydratedDocument<IClub>;
type EventDoc = HydratedDocument<IEvent>;
type RegistrationDoc = HydratedDocument<IRegistration>;
type AttendanceDoc = HydratedDocument<IAttendance>;
type NotificationDoc = HydratedDocument<INotification>;
...
const sampleOrganizers: SampleUser[] = [
  {
...
const sampleStudents: SampleUser[] = [
  {
...
const notificationTemplates = [
  ...
  {
    type: 'club_announcement' as const,
    channel: 'in_app' as const,
...
const ensureUser = async (sample: SampleUser): Promise<UserDoc> => {
  const passwordHash = await bcrypt.hash(sample.password, 10);
  const existing = await User.findOne({ email: sample.email }).exec();
...
const seedUsers = async () => {
  ...
  const existingAdmin = await User.findOne({ roles: 'admin', status: 'active' }).exec();
  if (existingAdmin) {
  ...
  return {
    admin: results.find((user) => user.roles.includes('admin')) as UserDoc,
    organizers: organizerDocs,
    students: studentDocs,
  };
};

const seedClubs = async (allUsers: UserDoc[]) => {
  ...
  const clubPromises = sampleClubs.map(async (club) => {
    ...
    const existingClub = await Club.findOne({ slug: club.slug }).exec();
...
const buildEventDate = (offsetDays: number) => {
  const date = new Date();
...
const seedEvents = async (organizers: UserDoc[], clubs: ClubDoc[]) => {
  console.log('[seed] Seeding events...');

  const clubMap = new Map(clubs.map((club) => [club.slug, club]));
  const eventDocs = [] as EventDoc[];
  ...
  for (const event of sampleEvents) {
    ...
    const existingEvent = await Event.findOne(titleFilter).exec();
    if (existingEvent) {
...
const seedRegistrations = async (students: UserDoc[], events: EventDoc[]) => {
  console.log('[seed] Seeding registrations...');

  const registrations: RegistrationDoc[] = [];
  ...
  while (registrations.length < 50) {
    ...
    const existingRegistration = await Registration.findOne({ eventId: event._id, userId: student._id }).exec();
...
const seedAttendance = async (registrations: RegistrationDoc[]) => {
  console.log('[seed] Seeding attendance records...');

  const attendanceRecords: AttendanceDoc[] = [];
  ...
  for (let index = 0; index < checkedInCount; index += 1) {
    ...
    const existingAttendance = await Attendance.findOne({ eventId: event._id, userId: registration.userId }).exec();
...
const seedNotifications = async (users: UserDoc[], events: EventDoc[], clubs: ClubDoc[]) => {
  ...
  for (let index = 0; notifications.length < 50; index += 1) {
    ...
    const existingNotification = await Notification.findOne(filter).exec();
...
const refreshEventAnalytics = async () => {
  ...
  for (const { _id, count } of registrationCounts) {
    await Event.findByIdAndUpdate(_id, { 'analytics.registeredCount': count }).exec();
  }
  ...
  for (const { _id, count } of attendanceCounts) {
    await Event.findByIdAndUpdate(_id, { 'analytics.checkedInCount': count }).exec();
  }
};
